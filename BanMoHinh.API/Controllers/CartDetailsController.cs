...
namespace BanMoHinh.API.Controllers
{
    ...
    public class CartDetailsController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("TangSl/")]
        public async Task<IActionResult> TangSl(Guid id, Guid idCartItem, string idgh)
        {
            try
            {
                var result = await _cartItemService.GetAllCartItemsByCartId(Guid.Parse(idgh));
                var lstSP = await _productDetailService.GetAll();
                var respon = result.FirstOrDefault(x => x.CartId.ToString() == idgh && x.ProductDetail_ID == id && x.Id == idCartItem);
        ...
        [HttpGet]
        [Route("GiamSL/")]
        public async Task<IActionResult> GiamSL(Guid id, Guid idCartItem, string idgh)
        {
            try
            {
                var result = await _cartItemService.GetAllCartItemsByCartId(Guid.Parse(idgh));
                ...
                {
                    var up = await _cartItemService.DeleteCartItem(idCartItem, id, Guid.Parse(idgh));
                    return new OkObjectResult(new { error = -1, message = "Sản phẩm tạm thời không còn" });
                }
                else
                {
